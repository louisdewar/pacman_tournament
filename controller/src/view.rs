...
use model::{BaseTile, Entity, Model};
...
impl View {
    ...
    fn build_terrain_mesh(&self, model: &Model, square_width: f32) -> graphics::MeshBuilder {
        let mut builder = graphics::MeshBuilder::new();

        for x in 0..(model.map().width as usize) {
            for y in 0..(model.map().height as usize) {
                let tex = tile_to_tex(&model.map().base_tile[x + y * model.map().width as usize]);
    ...
    pub fn draw(&mut self, ctx: &mut Context, model: &Model) -> GameResult {
